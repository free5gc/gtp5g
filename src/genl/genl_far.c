               ...
               struct gtp5g_emark_pktinfo *epkt_info)
{
    ...
    if (attrs[GTP5G_FORWARDING_PARAMETER_TRANSPORT_LEVEL_MARKING]) {
        param->hdr_creation->tosTc = nla_get_u8(attrs[GTP5G_FORWARDING_PARAMETER_TRANSPORT_LEVEL_MARKING]);
    }
        ...
        u32 type, struct far *far)
{
    ...
    if (fwd_param) {
        ...
        if (fwd_param->hdr_creation) {
            if (nla_put_u8(skb, GTP5G_FORWARDING_PARAMETER_TRANSPORT_LEVEL_MARKING, hdr_creation->tosTc))
                goto genlmsg_fail;
        }
